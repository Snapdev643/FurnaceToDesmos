...
class Compiler:
    ...
    def addHardcodedVals(self):
        ...
        try:
            with open("io/output.txt", "a") as file:
                num_orders = settings_instance.askNumOfOrders()
                total_length = num_orders * int(ft.find("pattern length")[18:])
                print(f"Total length of song is {total_length} rows.")
                if total_length > 1000:
                    print("Desmos cannot handle more than 1000 orders! Please reduce the number of orders in your Furnace file!")
                    return '\n"Error: Too many orders. Reduce the number of orders in your Furnace file."'
                #settings and time slider
                file.write('settings @{\nrandomSeed: "",\nviewport: @{ xmin: -10, xmax: 10, ymin: -6.5625, ymax: 6.5625 },\n}\n\n"Converted to Desmos by Antiblue\'s Furnace to Desmos Converter...  V2!!!"\n\nt = 1 @{ slider: @{ loopMode: "LOOP_FORWARD", period: 40000,'+ ' min: 1, max: ' + str(total_length) + ', step: 1/' + str(total_length) + ', },}\n\n')
                #note frequency data
